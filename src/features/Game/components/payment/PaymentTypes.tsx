...
const PaymentTypes = () => {
  ...
  // Contract Addresses
  const cfg = getChainConfig(42101);
  const DUNKVERSE_ADDRESS = cfg?.tokenContract as string;
  const BETTING_POOL_ADDRESS = cfg?.mainContract as string;
        ...
        <TabPanels className='mt-8'>
          ...
          <TabPanel>
            <div className='space-y-6'>
              ...
              <div className='grid grid-cols-2 items-center justify-between gap-2'>
                <span className='text-sm'>My Avail. Balance ($GTB)</span>
                <Button
  variant='outline'
  rightIcon={RxCopy}
  rightIconClassName='text-primary-500 text-xl'
  size='base'
  className='w-full px-5 py-3 text-white'
  onClick={() => handleCopy(`${Number(tokenBalance).toFixed(2)} $GTB`)}
  disabled={loading}
>
  <span className='mx-auto w-full'>
    {loading ? 'Loading...' : `${Number(tokenBalance).toFixed(2)} $GTB`}
  </span>
</Button>
              </div>
              <h2 className='!h2'>
  Bet Locked in BettingPool: {loading ? 'Loading...' : `${Number(totalInvested).toFixed(2)} $GTB`}
</h2>
              <Button 
                variant='outline' 
                size='lg' 
